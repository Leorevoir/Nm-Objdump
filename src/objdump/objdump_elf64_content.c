...
static void show_section_hex64(
    const Elf64_Shdr *sec, const unsigned char *buffer, size_t start)
{
    size_t i = start;
...
static void show_section_ascii64(
    const Elf64_Shdr *sec, const unsigned char *buffer,
...
static void show_section_remaining64(
    const Elf64_Shdr *sec, Elf64_Addr addr, const unsigned char *buffer)
{
    ...
    if (remaining != 0) {
        ...
        show_section_hex64(sec, buffer, last_start);
        show_section_ascii64(sec, buffer, last_start, remaining);
...
void show_elf64_sections_content(const Elf64_Shdr *sec, const objdump_t *obj)
{
    ...
    while (i < full_blocks) {
        show_section_hexadecimal(addr, FMT_04LX);
        addr += 16;
        show_section_hex64(sec, buffer, i);
        show_section_ascii64(sec, buffer, i, 16);
        console_log("%c", ENDLINE);
        i += 16;
    }
    show_section_remaining64(sec, addr, buffer);
}
